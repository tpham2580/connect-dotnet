...
internal sealed class FakeBusinessAppService : IBusinessService
{
    ...
    public Task<BusinessModel?> GetBusinessByIdAsync(long id) => Task.FromResult(GetByIdResult);

    public Task<List<BusinessModel>> GetAllBusinessesByIdsAsync(List<long> ids) =>
        Task.FromResult(new List<BusinessModel>());

    public Task<(List<BusinessModel> Businesses, long Total)> GetBusinessesAsync(int limit, int offset)
    {
        LastLimit = limit;
        LastOffset = offset;
    ...
    public Task<BusinessModel?> CreateBusinessAsync(BusinessModel business) => Task.FromResult(CreateResult);

    public Task<BusinessModel?> UpdateBusinessAsync(BusinessModel business) => Task.FromResult(UpdateResult);

    public Task<bool> DeleteBusinessByIdAsync(long id) => Task.FromResult(DeleteResult);
}
